...
const PRODUCT_NAME = PACKAGE_JSON.build?.productName || PACKAGE_JSON.productName || 'Vesper'
...
const APP = (() => {
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, EXECUTABLE_NAME.toLowerCase()),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `${PRODUCT_NAME}-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `${PRODUCT_NAME}-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}
