...
function Install-Desktop {
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Vesper.exe",
        "$desktopDir\release\win-arm64-unpacked\Vesper.exe",
    ...
    if (-not $found) {
        throw "Desktop build completed but no Vesper.exe or Hermes.exe was found under $desktopDir\release\*-unpacked\"
    }
